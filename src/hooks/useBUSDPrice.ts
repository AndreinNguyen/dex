import { Currency, JSBI, Price, WNATIVE, ChainId } from '@savvydex/sdk'
import { SVC, BUSD } from 'config/constants/tokens'
...
export default function useBUSDPrice(currency?: Currency): Price<Currency, Currency> | undefined {
  ...
  const wnative = WNATIVE[chainId] || WNATIVE[ChainId.MAINNET]
  const busd = BUSD[chainId] || BUSD[ChainId.MAINNET]
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    ...
    [busd, chainId, currency, wnative, wrapped],
  )
...
export const useCakeBusdPrice = (): Price<Currency, Currency> | undefined => {
  const { chainId } = useActiveWeb3React()
  const cakeBusdPrice = useBUSDPrice(SVC[chainId])
